  ...
  if (getInput("file_field", gml_filename)) {
    ...
    for (unsigned int i = 0; i != polys.size(); i++) {
      for (unsigned int j; j != polys[i].points.size(); j++) {
        nav2_complete_coverage_msgs::msg::Coordinate coord;
...
BT::NodeStatus ComputeCoveragePathAction::on_aborted()
{
  ...
  nav2_complete_coverage_msgs::msg::PathComponents cov_path;
...
BT_REGISTER_NODES(factory)
{
  ...
  factory.registerBuilder<nav2_coverage_bt::ComputeCoveragePathAction>(
